from src.modules.catalog.application.use_cases.create_themes.dto import CreateThemesInputDTO, CreateThemeInputDTO
...
class CreateThemesController(IControllerInterface):
    def __init__(self, create_themes_use_case: ICreateThemesUseCase):
    ...
    async def handle_request(self, request: HttpRequest) -> HttpResponse:
        ...
        created_themes_dto = await self.create_themes_use_case.execute(themes_data_dto)

        return HttpResponse(
            status_code=201,
            body=created_themes_dto.to_dict(created_themes_dto)
        )